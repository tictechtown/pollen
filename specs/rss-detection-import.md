...
## Action items
[ ] Add a discovery helper (e.g., `discoverFeedUrls(url)`): 
    - HEAD request to check `content-type`; if XML-ish, try `fetchFeed`.
    - Otherwise GET page, extract `<head>`, and parse `<link rel="alternate" type="application/rss+xml|application/atom+xml" href="...">` into absolute URLs.
[ ] Update `handleAdd` in `app/sources.tsx` to call discovery first, then:
    - If a direct feed: proceed with existing `fetchFeed` + DB upsert.
    - If multiple candidates: open a dialog with choices and only fetch after selection.
    - If none: show snackbar error.
